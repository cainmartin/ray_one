...
camera_new :: proc() -> Camera {
    return Camera { aspect_ratio = 1.0, image_width = 100 , samples_per_pixel = 100 }
}

camera_init :: proc(camera: ^Camera, width: int, ratio: f64, samples_per_pixel: int) {
...
get_ray :: proc(camera: ^Camera, col: int, row: int) -> Ray {
    ...
    pixel_sample := camera._pixel00_loc + ((f64(col) + offset.x) * camera._pixel_delta_u) + ((f64(row) + offset.y) * camera._pixel_delta_v)
